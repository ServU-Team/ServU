...
//  Product models for the ServU marketplace
//
...
struct Product: Codable, Identifiable {
    ...
    var priceRange: String {
        if variants.isEmpty {
            ...
            let min = minPrice
            let max = maxPrice
            return min == max ? displayPrice : "From \(String(format: "$%.2f", min))"
        }
    }
    
    var stockStatusText: String {
        if variants.isEmpty {
            return inventory.stockStatus.rawValue
        } else {
            let inStockVariants = variants.filter { $0.isInStock }
            if inStockVariants.isEmpty {
                return StockStatus.outOfStock.rawValue
            } else if inStockVariants.count < variants.count {
                return "Limited Availability"
            } else {
                return StockStatus.inStock.rawValue
            }
        }
    }
    
    var availableVariantCount: Int {
        return variants.filter { $0.isInStock }.count
    }
}